...
{0x1b, 0x00, 0x00, 0x00},  
{0x36, 0x00, 0x00, 0x00} };  
BYTE * CDSA::RotWord( BYTE *word )//左旋一位  
{  
...
BYTE *CDSA::SubWord( BYTE *word )  //用替换表 Sbox 对一给定的一行密钥调度表 w[] 进行逐字节替换。  
{  
	BYTE *result = new BYTE[4];  
...
CDSA::CDSA( ENUM_KEYSIZE keysize, BYTE *key )  
{  
...
void CDSA::Cipher( BYTE *input, BYTE *output )  
{  
...
void CDSA::InvCipher( BYTE *input, BYTE *output )  
{  
	int i;  
	if (this->State[0] == NULL)  
	{  
...
void CDSA::AddRoundKey( int round )  
{  
	for (int r = 0; r < 4; ++r)  
	{  
		for (int c = 0; c < 4; ++c)  
